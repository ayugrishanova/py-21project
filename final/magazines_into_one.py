...
for magazine in magazines:
    ...
    for article in filelist:
        ...
        if text.count(name) > 2:
            text_all += text
    list_articles.update({magazine: text_all})


    magazine_name = name + '_' + magazine + '.txt'
    with open(magazine_name, "w",encoding='utf-8') as write_file:
        print(text_all, file=write_file)
with open(f"{name}_corpus_magazines.json", "w") as write_file:
